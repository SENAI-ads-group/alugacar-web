				...
				<div class="block block-rounded">
					...
					<div class="block-content">
						<table class="table table-striped table-vcenter">
							...
							<tbody>
								<c:forEach var="tip" items="${ tipoAcessorioList }">
									<tr>
										<th class="text-center" scope="row">${ tip.id }</th>
										<td class="font-w600 font-size-sm"><a
											href="<c:url value="/tipos/${ tip.id }"/>">${ tip.descricao }</a></td>
										<td class="text-center">
											<div class="btn-group">
												<a class="btn btn-sm btn-alt-primary" data-toggle="tooltip"
													title="Editar"
													href="<c:url value="/tipos/${ tip.id }"/>"> <i
													class="fa fa-fw fa-pencil-alt"></i>
												</a>
												<form id="form-excluir" method="POST"
													action="<c:url value="excluir/${ tip.id }"/>">
